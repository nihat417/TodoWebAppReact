...
export default function Modal({ closeModal, modalType }) {
  return (
    ...
    <div className='fixed top-0 left-0 w-full h-full flex items-center justify-center z-99 bg-black bg-opacity-50 '>
      ...
      <h1 className='text-[1.875rem] font-bold'>{modalType === 'Create' ? 'Create' : 'Edit'} CARD</h1>
      ...
      {modalType === 'Delete' && (
        <div className='flex flex-col'>
            ...
            <button className='border border-zinc-300 py-2 px-5 w-[124px] h-[61px] rounded-[5px] font-bold hover:bg-[#DFDFDF]'
                onClick={() => {closeModal(false);}}>Close
            </button>
            <button className='bg-yellow-400 py-2 px-5 w-[124px] h-[61px] rounded-[5px] font-bold mx-2 hover:bg-[#FF4C4C]'>Delete</button>
            </div>
        </div>
      )}
    </form>
